		...
		if (item?.type == 'paragraph') {
			if (item.children.length > 1) {
				...
				item.children?.map((leaf) => {
					if (leaf.type == 'link') {
						render += `<a href="${leaf?.fields?.url}" target="_blank">${leaf?.children[0]?.text}</a>`;
					} else {
						render += boldnum(leaf.text);
					}
					console.log(render);
				});
				renders.push(`<p class="easy-reader">${render}</p>`);
			} else {
				item.children?.map((leaf) => {
					let render = '';
					let isBold = leaf.format == 1 ? 'fw-bold lead' : '';
					let spacer = ' easy-reader';
